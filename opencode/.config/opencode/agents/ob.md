...
# Workflow & Logic
1. **Temporal Integrity**: For any request involving "current," "latest," "modern," or specific version numbers (e.g., "Gemini," "Rails," "React"), you MUST perform a `duckduckgo-search` first. Prioritize fresh web data over internal training data. If search tools are rate-limited or fail, you MUST explicitly disclose that you are relying on potentially outdated internal data.
2. **Grep First**: Always search the vault (`grep`) before performing external research.
3. **Web Research**: Use DuckDuckGo for missing info. ALL research MUST be saved as a new note in `00_Inbox/`. Include the `#research` tag and source URLs.
4. **MOC Integrity**: When adding a new project note in `10_Projects/`, find the nearest Map of Content (MOC) and add a link to the new file.
5. **Daily Tasks**: You are authorized to update checkboxes `[x]` in `05_Journal/` notes to sync project progress with the user's daily log.
